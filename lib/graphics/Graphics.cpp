// Copyright 2018 ROie R. Black and others
#include "Graphics.h"
#include <time.h>
#include <cmath>
...
void drawCircle(int x, int y, int radius) {
     ...
     for (angle=0; angle< 2.0*PI + ANGLE_STEP; angle += ANGLE_STEP) {
         X = x + static_cast<int>(static_cast<double>(radius)
                 * cos(angle));
         Y = y + static_cast<int>(static_cast<double>(radius)
                 * sin(angle));
         glVertex2i(X, Y);
     }
     glEnd();
...
void drawFilledCircle(int x, int y, int radius) {
     ...
     for (angle=0; angle< 2.0*PI + ANGLE_STEP; angle += ANGLE_STEP) {
         X0 = X1;
         Y0 = Y1;
         X1 = x + static_cast<int>(static_cast<double>(radius)
                 * cos(angle));
         Y1 = y + static_cast<int>(static_cast<double>(radius)
                 * sin(angle));
         glVertex2i(x, y);
         glVertex2i(X0, Y0);
         glVertex2i(X1, Y1);
...
void drawOval(int x, int y, int width, int height, int rotateAngle) {
    ...
    glBegin(GL_LINE_STRIP);
    for (angle=0; angle< 2.0*PI + ANGLE_STEP; angle += ANGLE_STEP) {
        radius = static_cast<int>((a*b)/sqrt(
                    square(b*cos(angle-rotateAngleRad)) +
...
void drawFilledOval(int x, int y, int width, int height, int rotateAngle) {
    ...
    for (angle=0; angle< 2.0*PI + ANGLE_STEP; angle += ANGLE_STEP) {
        ...
        X0 = X1;
        Y0 = Y1;
        X1 = x + istatic_cast<int>(static_cast<double>(radius)
                * cos(angle));
        Y1 = y + static_cast<int>(static_cast<double>(radius)
                * sin(angle));
        glVertex2i(x, y);
...
void drawArc(int x, int y, int radius, int startAngle, int endAngle) {
    ...
    for (double angle = startRadians;
        ...
        X0 = x + static_cast<int>(static_cast<double>(radius)
                * cos(angle));
        Y0 = y + static_cast<int>(static_cast<double>(radius)
                * sin(angle));
        glVertex2i(X0, Y0);
    }
    glEnd();
