...
"""
    ...
    Include TiKz figures in Sphinx documents
...
# where to build the tikzfigure image
BUILD_TMPDIR = os.path.join(os.getcwd(),'latex','images')
if not os.path.isdir(BUILD_TMPDIR):
    os.makedirs(BUILD_TMPDIR)

class TikzFigureExtError(SphinxError): 
...
class RenderTikzFigureImage(object):
    ...
    def render(self):
        ...
        relfn = posixpath.join(self.builder.imgpath, 'TikzFigures',shasum)
        outfn = os.path.join(self.builder.outdir, self.builder.imagedir, 'TikzFigures', shasum)
        print("outfn:",outfn)
        print("relfn:",relfn)
        ...
        os.chdir(curpath)
        imagepath = os.path.join(os.path.abspath(self.imagedir),'TikzFigures')
...
class Figur(nodes.General, nodes.Element):
    pass

class TikzFigure(Directive):
    ...
    def run(self):
        ...
        # return the new node for display
        ret = [node]
...
def html_visit_tikzfigure(self, node):
    ...
    try:
        ...
        fname, relfn = RenderTikzFigureImage(latex, opts, libs, self.builder).render()
...
def latex_visit_tikzfigure(self, node):
    self.body.append('$' + node['latex'] + '$')
    raise nodes.SkipNode

def latex_visit_displayTikzFigures(self, node):
    self.body.append(node['latex'])
    raise nodes.SkipNode

def setup(app):
    app.add_node(TikzFigure,
        latex=(latex_visit_tikzfigure, None),
        html=(html_visit_tikzfigure,None))
    app.add_directive('tikzfigure', TikzFigure)
